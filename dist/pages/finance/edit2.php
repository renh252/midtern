  ...
  <div class="row mt-4">
    <div class="col-6">
      ...
      <form id="editForm" action="edit-api.php" method="POST" novalidate>
        ...
        <div class="mb-3" id="pet_id" style="display: none;">
...
<script>
  ...
  const petIdContainer = document.getElementById('pet_id');
...
<script>
  ...
  const sendData = e => {
    ...
    // 驗證手機號碼
    if (donor_name.value == 'Anonymous') {
      // 如果是匿名捐款，設置手機號碼和 Email 為 'Anonymous'
      phoneField.value = "Anonymous";
      emailField.value = "Anonymous";
      phoneField.removeAttribute('pattern');
    } else {
      // 不是匿名捐款，正常驗證手機號碼和 Email
      form.removeAttribute('novalidate'); // 如果不是 Anonymous，啟用驗證
