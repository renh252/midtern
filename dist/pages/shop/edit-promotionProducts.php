...
// 設定標題和頁面名稱
$title = "編輯活動商品";
$pageName = "edit-promotionProducts";
...
// ---------------------------
// 查詢 有搭配活動的商品
$stmt = $pdo->prepare("SELECT product_id, variant_id FROM promotion_products WHERE promotion_id = ?");
$stmt->execute([$promotion_id]);
$promotionProducts = $stmt->fetchAll(PDO::FETCH_ASSOC);

// 準備要傳給 JavaScript 的數據
$selectedProducts = [];
$selectedVariants = [];
foreach ($promotionProducts as $product) {
    ...
    if ($product['variant_id']) {
        $selectedVariants[] = $product['variant_id'];
    }
}

// 將數據轉換為 JSON 並輸出到 JavaScript
// --------------------------
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-end">
                                <li class="breadcrumb-item"><a href="promotions.php">促銷活動
                                </a></li>
...
var selectedVariants = <?= json_encode($selectedVariants) ?>;
document.addEventListener('DOMContentLoaded', function() {
    // 預選商品
    selectedProducts.forEach(function(productId) {
        var checkbox = document.querySelector('input[name="product_id[]"][value="' + productId + '"]');
        if (checkbox) {
    ...
    selectedVariants.forEach(function(variantId) {
        var checkbox = document.querySelector('input[name="variant_id[]"][value="' + variantId + '"]');
        if (checkbox) {
            checkbox.checked = true;
        }
    });
